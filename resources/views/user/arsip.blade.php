...
<div class="produk-page">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
            <div>
                <h1>
                    ...
                    Arsip Karyawan (Dihapus)
                </h1>
                <p>Daftar karyawan yang telah dihapus dari sistem</p>
            </div>
            <div>
    ...
    <div class="am-table-wrap">
        <div class="am-table-toolbar">
            <form method="GET" action="{{ route('users.arsip') }}" class="am-search-bar">
                <input type="text" name="search" placeholder="Cari nama/username..." value="{{ $search ?? '' }}">
                <button type="submit" class="btn btn-primary btn-sm">Cari</button>
                @if (!empty($search))
                    <a href="{{ route('users.arsip') }}" class="btn btn-default btn-sm">Reset</a>
                @endif
            </form>
            <span class="text-muted" style="font-size:13px;">{{ $datas->total() }} karyawan di arsip</span>
        </div>
        ...
        <div class="produk-table-box">
            <table class="table produk-table">
                <thead>
                    <tr>
                        <th style="width: 30%;">Nama & Username</th>
                        <th style="width: 25%;">Email & No HP</th>
                        <th style="width: 20%;">Tipe User</th>
                    ...
                    @forelse ($datas as $d)
                        <tr>
                            <td>
                                ...
                                <span class="produk-desc">{{ $d->username ?? '-' }}</span>
                                <span class="produk-desc" style="color:#ef4444; margin-top:8px;">Dihapus pada: {{ $d->deleted_at->format('d/m/Y H:i') }}</span>
                            </td>
                            <td>
                                <span class="produk-name">{{ $d->email ?? '-' }}</span>
                                <span class="produk-desc">{{ $d->no_hp ?? '-' }}</span>
                            </td>
                            <td>
                        ...
                        <tr>
                            <td colspan="4">
                                <div class="am-empty">
                                    ...
                                    <p>Tidak ada karyawan di dalam arsip / tempat sampah.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
